...
@onready var _islands_gen:Node = $islands;
...
func _Generate_Map(newSeed:int) -> void:
	...
	# Generate Map Data
	_terrain_data = Map_Data.Derive_TerrainData_From_NoiseData(_mountain, _highland, _ground, _shore, _shallow, _sea, _ocean, _depths, noiseData);
	# Add Docks
	...
	# Derive Islands
	#_islands = _islands_gen.Islands_From_TerrainData(_terrain_data, true, true, self.get_path());
...
#
	...
	#for island in _islands:
		#var col:Color = Color(randf_range(.25, 1), randf_range(.25, 1), 0);
		#for idx in island:
	...
	for m in markingData:
		...
		match m:
			...
			Map_Data.Marking.HILL:
				#spr = _Create_Sprite(5, 6, _spriteSheet);
				#_cont_showOnZoom.add_child(spr);
				#spr.modulate = Color.BLACK;
				#spr.scale /= 1.5;
				s_array.append(null);
